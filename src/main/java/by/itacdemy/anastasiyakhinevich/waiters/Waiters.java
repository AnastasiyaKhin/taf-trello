...
public class Waiters {
    ...
    public static void waitForLoad() {
        WebDriverWait wait = new WebDriverWait(Driver.getThreadLocalDriver(), Duration.ofSeconds(8));
        wait.until((ExpectedCondition<Boolean>) wd -> ((JavascriptExecutor) wd).executeScript("return document.readyState").equals("complete"));
        log.info("page is load");
    }

    public static WebElement waitForElementToBeClickable(WebElement webElement) {
        return new WebDriverWait(Driver.getThreadLocalDriver(), Duration.ofSeconds(WAIT_TIMEOUT_SECONDS)).until(ExpectedConditions
